    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            if not allow or env.settings in allow:
                ...
                warn('Skipping "%s", no environment specified' % func.__name__)
...
def require_static_settings(fn=None, **kwargs):
    """
    Decorator preventing function from running unless env.static_settings is set.
    If not set, an error will be thrown via fabric.api.require().
    
    Available options:  
    
    allow:     Only run the function if env.static_settings is in this list.    
    verbose:   Output a warning to the user about skipped function.
    
    Examples:
    
    @require_static_settings
    def f():

    @require_static_settings(allow=['loc','stg'], verbose=True)
    def f():
    """ 
    allow = kwargs.pop('allow', None)
    verbose = kwargs.pop('verbose', False)   
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            require('static_settings')
            if not allow or env.static_settings in allow:
                return func(*args, **kwargs)
            elif verbose:
                warn('Skipping "%s", no environment specified' % func.__name__)
        return update_wrapper(wrapper, func)
